...
def parse_gm_map(path):
    """fingerprint(str) -> dict(program, transpose, volume, cents)."""
    out = {}
    with open(path) as f:
        for line in f:
            s = line.strip()
            ...
            cents = int(parts[4]) if len(parts) >= 5 else 0
            out[fp] = {"program": prog, "transpose": transpose, "volume": volume, "cents": cents}
    return out
    ...
    for i, nt in enumerate(same):
        ...
        for j in range(len(same)):
            ...
            if (o.off <= nt.on and o.off > nt.on - GUARD_TICKS
                ...
                break
        if not overlap:
            cand.append((nt.on, nt.off, nt.note, nt.opl_hz))
...
def build_window_specs(windows, sr, total_samples, transpose=0):
    ...
    A soundfont preset that reproduces the OPL original's true pitch reads
...
def write_solo_map(all_fps, gm_map, target_fp, path):
    """Every fingerprint at volume 0 except target at its mapped program/vol/cents."""
    with open(path, "w") as f:
        f.write("# solo pitch-check map for %s (auto-generated by validate_pitch.py)\n" % target_fp)
        for fp in all_fps:
            m = gm_map.get(fp, {"program": 0, "transpose": 0, "volume": 100, "cents": 0})
            if fp == target_fp:
                vol = 100
            else:
                vol = 0
            f.write("%s %d %d %d %d\n" % (fp, m["program"], m["transpose"], vol, m.get("cents", 0)))
...
def write_report(a_result, b_result, b_error, gm_map, notes):
    ...
    if b_result is None:
        ...
        L.append("and compared to the note's OPL-referenced expected frequency (the exact "
                 "OPL2 chip frequency at note-on, shifted by the map's transpose column; "
                 "the map's cents fine-tune acts on the render, not the reference) rather "
                 "than the equal-tempered MIDI grid -- perfect fidelity to "
                 "the original reads as 0.00 st here. Offset = "
