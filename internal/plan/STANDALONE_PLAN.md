...
**S3c — estsc.shp DONE, tshp2.pcx consciously deferred:**
- **`estsc.shp` (ending/credits corner pics): wired** in `JE_playCredits`
  (`mainint.c`). The credits screen has no full-screen picture to hang the HD
  overlay on, so it activates HD compositing with a flat-black filler backdrop
  (`hdpcx_credblack.dat`, generated by `tools/hd_extract.py`
  `write_credits_backdrop_filler()`) purely to unlock the `hd_set_sprite` queue,
  maps the classic 0–15 crossfade onto `hd_backdrop_fade`, queues `hdextra_NN.dat`
  per frame (per-sprite classic fallback), and bakes the last pic classically
  before `fade_black`. Fully guarded: `hd_credits` false ⇒ classic path
  byte-identical. Builds clean; **HD credits visual is headlessly unreachable —
  needs a manual playtest of the ending credits.**
- **`tshp2.pcx` (interlude): deferred, ships classic.** Its only call site is the
  `'P0'` flow command inside `JE_loadMap()` — off-limits (owned by the HD-tileset
  work). `hdpcx_tshp2.dat` already exists; wiring needs an enter/exit audit of
  `JE_loadMap`'s picture-changing branches first. Deferral documented in
  `pcxload.c`.
