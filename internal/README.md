...
- [plan/CONTROLLER_PLAN.md](plan/CONTROLLER_PLAN.md) — replace the raw `SDL_Joystick` subsystem with `SDL_GameController` (Xbox/PlayStation named buttons, hotplug, remap screen); records the settled decisions (pure replace, no legacy-stick fallback), the default key map, and the SDL traps (ADDED=device index vs REMOVED=instance id, double-add on init, menu label width budget)
...
- [issue/hd-music-untagged-loop-silence.md](issue/hd-music-untagged-loop-silence.md) — **Fixed (hd-remaster)** — HD music fell silent mid-mission on 4 looping tracks whose OGGs lacked LOOP tags; engine now loops untagged tracks while the LDS master is still playing (no asset change)
