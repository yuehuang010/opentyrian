...
void JE_paramCheck(int argc, char *argv[])
{
	const Options options[] =
	{
		...
		// Hidden: levels<ep>.dat section-ordinal retargeting self-test
	...
	for (; ; )
	{
		...
		case 264: // --edit-script-retarget-test (hidden)
		{
			int temp = atoi(option.arg);
			if (temp >= 1 && temp <= 4)
				edit_script_retarget_episode = temp;
			else
			{
				fprintf(stderr, "%s: error: invalid --edit-script-retarget-test episode (expected 1-4)\n", argv[0]);
				exit(EXIT_FAILURE);
			}
			break;
		}
